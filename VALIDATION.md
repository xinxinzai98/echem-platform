...
- 8 项 `0.3.0-dev.11` 文件范围、补偿安全默认值、稳定数据段和分析质量门测试
...
- 分析页默认只显示实验数据；`Control_Programs`、`diagnostics` 和 `Rejected_Data`
  被隔离到“其他文件”，仍可按需检视但不会占据默认首屏
...
- Mac 完整回归共 119 项，全部通过；Python 编译、前端 JavaScript 语法和补丁
  空白检查均通过

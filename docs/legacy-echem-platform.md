...
桌面工作台、数据文件夹视图、方法分析与 60 秒 OCP 安全门版本：0.3.0-dev.12
...
- 在 `/start-stop` 下载或上传启停/ADT 数据，查看数据概况，并管理材料名称、备注与入图选择
- 在 `/start-stop/analysis` 的“稳定性分析”中分别查看启停循环与恒流长期运行；蓝博数据按内嵌工步和实测电流波动自动分类
- 蓝博原始 `.bts` 字节与标准化曲线分别入库，保留父文件 SHA-256、解析器版本、分类依据、电极面积和未确认的电压参照边界
- 配置页可下载 Matplotlib 生成的完整图集和水位补偿图集；分析页可单独导出当前检查视图
...
该程序由 `start_stop_service.py` 独立启动，不加载原电化学平台的 `app.py`，也不提供工作台、
工步、通用数据分析或仪器控制入口。Docker 镜像只包含以下两项能力：

1. 从三台实验电脑只读增量下载已稳定至少 5 分钟的数据文件，并从蓝博八通道电脑导入稳定 BTS 原始数据；
2. 管理材料名称与入图选择，在稳定性分析中分别完成启停分析和恒流分析、交互查看与导出。

蓝博分类优先使用 BTS 内嵌工步；如果内嵌工步只有一个电流档，还会用实测电流的
P5–P95 跨度、持续占比和重复档位切换进行复核。存在两个以上持续且显著不同电流档的
数据进入启停分析，单一电流档或没有持续重复切换的数据进入恒流分析。蓝博电压参照
确认前始终单独绘图，不与 Hg/HgO 曲线共用纵轴。

镜像不包含仪器控制、协议编辑、通用文件浏览、通用分析或其他平台页面。原始文件按
SHA-256 以 SQLite BLOB 保存到 `state/docker/database/start-stop.sqlite3`，同路径更新会保留
不可变版本；分析只读取冻结的候选快照。旧原始数据目录不会挂载到容器，也不会在采集或
迁移时被修改。

同一镜像运行两个隔离入口：

- 本机入口默认发布到 `127.0.0.1:18787`，可以下载数据、保存材料配置和重新绘图；
- 局域网入口默认发布到 `192.168.110.158:18788`，只允许查看启停结果与下载 PDF，
  所有 POST、PUT、PATCH、DELETE 都返回 403。

只有本机容器以单文件只读方式挂载三把 SSH 密钥和 `known_hosts`；局域网容器不挂载
数据库、SSH 密钥或原始数据，只读共享已经发布的分析缓存。两个容器均使用只读根文件系统、
移除全部 Linux capabilities，并启用 `no-new-privileges`。

首次构建：

```sh
./scripts/docker-compose.sh config --quiet
./scripts/docker-compose.sh build --pull
```

启动测试部署不会影响正在使用的原生 `8787` 页面：

```sh
ECHEM_LOCAL_PORT=18787 \
ECHEM_LAN_BIND=0.0.0.0 \
ECHEM_LAN_IP=192.168.110.158 \
ECHEM_LAN_PORT=18788 \
  ./scripts/docker-compose.sh up -d
```

Docker Desktop for macOS 通过 `0.0.0.0:18788` 发布端口，应用仍严格校验浏览器使用的
`192.168.110.158:18788` Host，且该入口始终只读。Compose 设置了
`restart: unless-stopped`；重建或重启容器不会清空数据库、材料配置或分析产物。除非先完成
验收并明确确认切换，否则不要占用原生服务使用的 `8787`。
...
从两个不同入口继续：[通用数据工作台入口](</Users/hive/Desktop/20260724 科学平台/app.py>)；[独立启停服务入口](</Users/hive/Desktop/20260724 科学平台/start_stop_service.py>)。这里只说明文件角色，不表示服务当前运行。

| 位置 | 用途与保留规则 |
| --- | --- |
| [state](</Users/hive/Desktop/20260724 科学平台/state>) | 持久数据库、状态及备份；不当作构建缓存 |
| [diagnostics](</Users/hive/Desktop/20260724 科学平台/diagnostics>) | 诊断与真实样例证据；逐项保留来源 |
| [scripts](</Users/hive/Desktop/20260724 科学平台/scripts>) | 采集、处理与复现脚本 |
| [output](</Users/hive/Desktop/20260724 科学平台/output>) | 含浏览器检查与审查输出；不整目录删除 |
| [outputs](</Users/hive/Desktop/20260724 科学平台/outputs>) | 按任务形成的报告/工作簿等派生交付；保留源与参数 |
| [artifacts](</Users/hive/Desktop/20260724 科学平台/artifacts>) | 分析图与审查产物混合；真实/模拟以对应记录为准 |
| [deliverables](</Users/hive/Desktop/20260724 科学平台/deliverables>) | 部署包、示意图与技术交付；名称不代表已验收 |

以上目录保持原位；本导航不统一移动到新的outputs体系，不触发采集、数据库迁移或仪器操作。

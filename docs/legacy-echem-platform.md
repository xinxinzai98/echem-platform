...
从两个不同入口继续：[通用数据工作台入口](../app.py)；[独立启停服务入口](../start_stop_service.py)。这里只说明文件角色，不表示服务当前运行。

| 位置 | 用途与保留规则 |
| --- | --- |
| [state](../state) | 持久数据库、状态及备份；不当作构建缓存 |
| [diagnostics](../diagnostics) | 诊断与真实样例证据；逐项保留来源 |
| [scripts](../scripts) | 采集、处理与复现脚本 |
| `output/` | 含浏览器检查与审查输出；不整目录删除 |
| `outputs/` | 按任务形成的报告/工作簿等派生交付；保留源与参数 |
| `artifacts/` | 分析图与审查产物混合；真实/模拟以对应记录为准 |
| `deliverables/` | 部署包、示意图与技术交付；名称不代表已验收 |
